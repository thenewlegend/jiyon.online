<script lang="ts">
  ...
  class Particle {
    ...
    constructor(width: number, height: number) {
      ...
      this.speedY = Math.random() * 2; // Slightly faster rise
      this.speedX = Math.random() - 0.1; // Wobble
      this.opacity = Math.random() * 0.8; // Higher base opacity
    }
    ...
    draw(ctx: CanvasRenderingContext2D) {
      ...
      // Vibrant, saturated cyan for Green Energy Hydrogen contrast
      ctx.fillStyle = `rgba(0, 220, 255, ${this.opacity})`;
      ctx.fill();
    }
  }
  ...
  onMount(() => {
    ...
    const particleCount = Math.min(Math.floor(window.innerWidth) * 100, 3000);
