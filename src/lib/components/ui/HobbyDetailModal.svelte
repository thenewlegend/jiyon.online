    ...
    <div class="space-y-10">
      ...
      <div class="space-y-4">
        <div class="flex items-center gap-3">
          ...
          <span class="text-[10px] uppercase font-bold tracking-[0.3em] text-[var(--color-accent)]">
            Hobby Showcase
          </span>
        </div>
        <Heading level={2} text={hobby.title} class="text-4xl lg:text-5xl" />
        <Text text={hobby.description} class="text-sm opacity-60 leading-relaxed" />
      </div>

      <!-- Works Gallery -->
      <div class="space-y-6">
        <div class="flex items-center justify-between border-b border-[var(--color-base-muted)]/10 pb-4">
          <span class="text-[10px] uppercase font-bold tracking-widest text-[var(--color-base-muted)]">
            Selected Works
          </span>
          <span class="text-[10px] uppercase font-mono text-[var(--color-accent)]">
            ...
            {#each hobby.works as work, i (work.url)}
              ...
              >
                ...
                <div class="p-4 flex items-center justify-between">
                  <span class="text-xs font-medium tracking-wide text-[var(--color-base-light)]">
                    {work.label}
                  </span>
                  ...
                  {:else}
                    <div class="w-1.5 h-1.5 rounded-full bg-[var(--color-accent)] opacity-40"></div>
                  {/if}
                </div>
              </div>
            {/each}
          {:else}
            <div class="py-12 text-center border border-dashed border-[var(--color-base-muted)]/20 rounded-sm">
              <Text text="Showcase content coming soon." muted class="text-xs italic" />
            </div>
          {/if}
        </div>
      </div>
      ...
      <!-- Footer -->
      <div class="pt-6 border-t border-[var(--color-base-muted)]/10 flex justify-between items-center opacity-40">
        <span class="text-[9px] uppercase tracking-[0.2em] font-mono italic">
          Personal Archive // {new Date().getFullYear()}
        </span>
        <div class="flex gap-1">
          <div class="w-1 h-1 rounded-full bg-[var(--color-accent)]"></div>
          <div class="w-1 h-1 rounded-full bg-[var(--color-accent)] opacity-50"></div>
