...
>
  <div class="flex-grow space-y-4 lg:space-y-6">
    <div
      class="flex items-center gap-4 text-[10px] uppercase font-bold tracking-[0.2em] opacity-60
      {lightMode
        ? 'text-[var(--color-base-dark)]'
        : 'text-[var(--color-base-muted)]'}"
    >
      <span>{post.date}</span>
    ...
    <Heading
      ...
      class="text-xl lg:text-3xl transition-colors duration-300 leading-tight line-clamp-3
        {lightMode
        ? 'text-[var(--color-base-dark)] group-hover:text-black'
        : 'text-[var(--color-base-light)] group-hover:text-[var(--color-accent)]'}"
    />
    ...
    <Text
      ...
      class="text-xs lg:text-sm line-clamp-3 font-light
        {lightMode
        ? 'text-[var(--color-base-dark)]/70'
        : 'opacity-60 text-[var(--color-base-light)]'}"
    />
    ...
    {#if post.tags}
      <div class="flex flex-wrap gap-2 pt-2">
        {#each post.tags as tag}
          <span
            class="text-[9px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-sm border
            {lightMode
  ...
  <div
    ...
    {lightMode
      ? 'border-[var(--color-base-dark)]/10'
      : 'border-[var(--color-base-muted)]/10'}"
  >
    <span
      class="text-[10px] uppercase font-bold tracking-[0.3em] transition-colors
      {lightMode
        ? 'text-[var(--color-base-dark)]/60 group-hover:text-black'
        : 'text-[var(--color-base-muted)] group-hover:text-[var(--color-accent)]'}"
    >
      Read Insight
    </span>
    <div
      class="w-8 h-8 flex items-center justify-center border rounded-full transition-all duration-500 transform group-hover:rotate-45
      {lightMode
